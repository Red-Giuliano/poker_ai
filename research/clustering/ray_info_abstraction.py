...
def get_card_combos(evals, num_cards: int) -> np.ndarray:
    ...
    """
    return np.asarray(list(combinations(evals, num_cards)))


def create_info_combos_fast(start_combos:np.array, publics:np.array, board_len:int) -> np.ndarray:
    """
    Combinations of private information (hole cards) and the entire deck
    ...
    :param publics: np.array of all available cards
    :param board_len : how many cards to sample
    ...
    for combos in tqdm(start_combos):
        combos = combos
        temp_evals1 = np.delete(publics, np.where(publics==combos[0])) 
        temp_evals = np.delete(temp_evals1, np.where(temp_evals1==combos[1]))
        boards = get_card_combos(temp_evals,board_len)
        combo_vec = np.stack([combos for _ in range(boards.shape[0])])
        #print(combo_vec)
        #print(boards)
        #print(combo_vec.shape)
        #print(boards.shape)
        our_cards.append(np.concatenate((combo_vec,boards),axis=1))
    arr = np.asarray(our_cards)
...
class InfoSets:
    ...
    def __init__(self):
        ...
        self.starting_hands = get_card_combos(self._evals,2)
        self.flop = create_info_combos_fast(self.starting_hands,self._evals, 3)
        self.turn = create_info_combos_fast(
            self.starting_hands, self._evals, 4
        )  
        self.river = create_info_combos_fast(
            self.starting_hands, self._evals,5
        )  
